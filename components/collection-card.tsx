      ...
      <CardContent className="p-4">
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1">
            <h3 className="font-medium text-balance leading-tight">{getSpeciesLabel(event.species)}</h3>
            <p className="text-sm text-muted-foreground mt-1">ID: {event.eventId}</p>
          </div>
        ...
        <div className="space-y-2 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-xs">{formatLocation(event.location.lat, event.location.lon)}</span>
          </div>
        ...
        <div className="flex gap-2 mt-4">
          ...
          <Button variant="outline" size="sm" onClick={() => onSendSMS(event)}>
